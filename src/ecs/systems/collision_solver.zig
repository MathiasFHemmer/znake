...
pub fn collision_solver(world: *World) void {
    ...
    for (query.collider.dense.items, query.collider.entities.items, 0..) |*collider, entity, idx| {
        for ((idx + 1)..query.collider.dense.items.len) |other_idx| {
            ...
            if (shapes.checkCollision(transform.position, transform.rotation, other_transform.position, other_transform.rotation, collider.shape, other_collider.shape)) |manifest| {
                ...
                const percent = 1.1; // usually 20% of penetration
